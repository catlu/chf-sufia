Blacklight.onLoad(function() {
  ...
  function get_autocomplete_opts_qa(field) {
    var autocomplete_opts = {
      minLength: 2,
      source: function( request, response ) {
        $.getJSON( "/qa/search/local/" + field, {
          q: request.term
        }, response );
      },
      focus: function() {
  ...
  // hard-code subject to use new fast qa endpoint
  $("#generic_file_subject")
    ...
    .autocomplete( get_autocomplete_opts_qa('fast' ));
  ...
  function setup_autocomplete(e, cloneElem) {
    ...
    if ($cloneElem.attr("id") == 'generic_file_subject') {
      $cloneElem.autocomplete(get_autocomplete_opts_qa('fast' ));
